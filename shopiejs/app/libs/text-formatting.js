...
export function splitLines(x, keepend = false) {
  return keepend ? x.split(/(?=[\r?\n])/g) : x.match(/[^\r?\n]+/g);
}

export var downcode = (function () {
  const LATIN_MAP = {
...
export default function URLify(s, num_chars) {
  s = downcode(s);
  var r = new RegExp('\\b(' + REMOVE_LIST.join('|') + ')\\b', 'gi');
  s = s.replace(r, '');
...
export function titleize(input) {
  var words = input.split(' ').map(function (word, index) {
    if (index === 0 || REMOVE_LIST.indexOf(word) === -1) {
      word = capFirst(word);
    }

...
class BaseModifier(object):
    """An interface for class that do modify to cart such as addition or not.
    The common ecommerce do is add tax amount or discount.
    ...
    def process_cart_item(self, cart_item, request):
        ...
        if field:
            ...
            cart_item.extra_fields.append(field)
        return cart_item
    ...
    # These 2 method expected return 2 tuple, the one is label and the second is
    # amount. If you decided no operation then you should return falsely - any
    # Python type that considered as false value.
...
class CartModifierPool(object):
    ...
    def _load_modifiers_list(self):
        ...
        if not getattr(settings, 'SHOP_CART_MODIFIERS', None):
            return result
        #iterate it
        for mod in settings.SHOP_CART_MODIFIERS:
            try:
                mod_module, mod_classname = mod.rsplit('.', 1)
            except ValueError:
                raise exceptions.ImproperlyConfigured(
                        '%s isn\'t a price modifier module' % (mod)
                    )
            try:
